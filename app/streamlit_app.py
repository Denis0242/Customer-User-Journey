...
# ---------------------------------------------------------
st.markdown(
    '<div class="main-title">Customer User Journey Analytics Dashboard</div>',
    unsafe_allow_html=True
)
st.markdown(
...
# KPI Calculations
# ---------------------------------------------------------
awareness_users = filtered.loc[
    filtered["funnel_stage"] == "Awareness",
    "customer_id"
].nunique()

purchase_users = filtered.loc[
    filtered["funnel_stage"] == "Purchase",
    "customer_id"
].nunique()
...
cohort["cohort_month"] = (
    cohort.groupby("customer_id")["journey_date"]
    .transform("min")
    .dt.to_period("M")
    .astype(str)
)
...
cohort_table["retention_rate"] = (
    cohort_table["active_users"] / cohort_table["cohort_size"]
)
...
# ---------------------------------------------------------
# TABLES AFTER VISUALS
# ---------------------------------------------------------
st.subheader("Customer Segment Performance Table")

segment_table = (
    filtered.groupby(
        ["customer_segment", "acquisition_channel", "churn_risk_category"]
    )
    .agg(
        customers=("customer_id", "nunique"),
        revenue=("revenue", "sum"),
...
segment_table["d1_retention"] = (
    segment_table["d1_retention"] * 100
).round(1).astype(str) + "%"

segment_table["d7_retention"] = (
    segment_table["d7_retention"] * 100
).round(1).astype(str) + "%"

segment_table["revenue"] = segment_table["revenue"].round(0)
segment_table["avg_revenue_per_customer"] = (
    segment_table["avg_revenue_per_customer"].round(2)
)
...
# ---------------------------------------------------------
# EXECUTIVE DECISION SUMMARY AT THE BOTTOM
# ---------------------------------------------------------
...
i1.markdown(
    f"<div class='insight'><b>Insight</b><br>{insight_text}</div>",
    unsafe_allow_html=True
)

i2.markdown(
    f"<div class='action'><b>Action</b><br>{action_text}</div>",
    unsafe_allow_html=True
)

i3.markdown(
    f"<div class='recommendation'><b>Recommendation</b><br>{recommendation_text}</div>",
    unsafe_allow_html=True
)

i4.markdown(
    f"<div class='decision'><b>Decision</b><br>{decision_text}</div>",
    unsafe_allow_html=True
)